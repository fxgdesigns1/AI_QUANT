...
class OrderManager:
    ...
    def calculate_position_size(self, signal: TradeSignal) -> Optional[PositionSizing]:
        ...
        try:
            ...
            current_price = signal.stop_loss or 0.0
            if signal.side == OrderSide.BUY:
                stop_loss_distance = abs(current_price - signal.stop_loss) if signal.stop_loss else 0.001
            else:
                stop_loss_distance = abs(signal.stop_loss - current_price) if signal.stop_loss else 0.001
            ...
            if stop_loss_distance > 0:
                units = int(risk_amount / stop_loss_distance)
            else:
                units = signal.units  # Fallback to signal units

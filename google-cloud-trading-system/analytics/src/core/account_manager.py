...
class AccountManager:
    ...
    def _load_account_configs(self):
        ...
        # Gold Scalping Account (Account 010) - Ultra Strict Forex Strategy
        gold_account = AccountConfig(
            ...
            risk_settings={
                ...
                'max_positions': int(os.getenv('GOLD_MAX_POSITIONS', '15')),  # REDUCED - stop overtrading
                'daily_trade_limit': int(os.getenv('GOLD_DAILY_TRADE_LIMIT', '100'))
            },
            instruments=['EUR_USD', 'GBP_USD', 'USD_JPY', 'AUD_USD']  # Forex pairs for Ultra Strict
        )
        
        # Strategy Alpha Account (Combined Portfolio) - CORRECTED
        alpha_account = AccountConfig(
            account_id=os.getenv('STRATEGY_ALPHA_ACCOUNT'),
            account_name="STRATEGY_ALPHA",
            api_key=os.getenv('OANDA_API_KEY'),
            environment=os.getenv('OANDA_ENVIRONMENT', 'practice'),
            strategy_name="momentum_trading",  # CORRECTED: Was alpha
            risk_settings={
                'max_risk_per_trade': float(os.getenv('ALPHA_MAX_RISK_PER_TRADE', '0.025')),
                'max_portfolio_risk': float(os.getenv('ALPHA_MAX_PORTFOLIO_RISK', '0.75')),  # CORRECTED: Was 0.12
                'max_positions': int(os.getenv('ALPHA_MAX_POSITIONS', '7')),
                'daily_trade_limit': int(os.getenv('ALPHA_DAILY_TRADE_LIMIT', '30'))
            },

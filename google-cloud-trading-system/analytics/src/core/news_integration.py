...
class SafeNewsIntegration:
    ...
    def __init__(self):
        ...
        self.update_interval = 600  # 10 minutes - increased to reduce API calls
        self.api_call_times = {}  # Track API call times for rate limiting
        self.rate_limits = {
            'alpha_vantage': 60,  # 1 call per minute
            'marketaux': 60,      # 1 call per minute
            'newsdata': 60,       # 1 call per minute
            'newsapi': 60         # 1 call per minute
        }

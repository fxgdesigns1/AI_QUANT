#!/usr/bin/env python3
"""
Fixed 14-Day Backtest for All Strategies
Tests each strategy against the past 14 days of market data
Properly handles candle data format and respects fundamental strategy characteristics
Win rate < 50% = FAILURE
"""
...
def get_historical_data(client, instrument, days=14, granularity='M5'):
    """Get historical data from OANDA with proper error handling"""
    ...
    try:
        ...
        for candle in candles:
            if 'time' not in candle or 'mid' not in candle:
                continue
                
            try:
                timestamp = candle['time']
                if isinstance(candle['mid'], dict):
                    open_price = float(candle['mid'].get('o', 0))
                    high_price = float(candle['mid'].get('h', 0))
                    low_price = float(candle['mid'].get('l', 0))
...
def prepare_market_data(df_row, instrument):
    ...
    return MarketData(
        instrument=instrument,
        timestamp=timestamp,
        bid=close_price - 0.0001,  # Simulate bid/ask spread
...
def calculate_technical_indicators(df):
    ...
    # Initial values
    smoothed_tr[0] = np.sum(tr[:period])
    smoothed_plus_dm[0] = np.sum(plus_dm[:period])
    smoothed_minus_dm[0] = np.sum(minus_dm[:period])
    ...
    atr[0] = tr[0]  # Initial ATR is the first TR
                ...
                if hasattr(strategy, 'price_history'):
                    if isinstance(strategy.price_history, dict):
                        if instrument not in strategy.price_history:
                            strategy.price_history[instrument] = []
                        strategy.price_history[instrument].append(float(df_at_time['close'].iloc[0]))
                    else:
                        strategy.price_history.append(float(df_at_time['close'].iloc[0]))

...
class ExecutionGate:
    ...
    def decision(self) -> GateDecision:
        ...
        if mode == "live":
            ...
            if not (a and b):
                # Log a clear human-readable message for gate blocking
                missing = []
                if not a:
                    missing.append("LIVE_TRADING")
                if not b:
                    missing.append("LIVE_TRADING_CONFIRM")
                print(f"⚠️  EXECUTION GATE: Live mode requested but blocked - missing: {', '.join(missing)}. Trades will be simulated (paper mode).")

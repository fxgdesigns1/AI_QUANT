...
import {
  CriticalCanonicalBridgePanel,
  WindowsSidecarPanel,
  BridgeComparisonTable,
  BridgeSwitchStatePanel,
  BridgeFailureBucketPanel,
...
export default function ControlPlaneDashboard() {
  ...
  // Runner runtime controls (token-protected PATCH)
  const [adminToken, setAdminToken] = useState('');
  const [controlsError, setControlsError] = useState(null);
  const [riskCaps, setRiskCaps] = useState(null);
  const [tradeSelection, setTradeSelection] = useState(null);
  const [accountLanes, setAccountLanes] = useState(null);
  const [commandPackText, setCommandPackText] = useState(null);
  ...
  const [tradeSelForm, setTradeSelForm] = useState({
    mode: '',
    daily_trade_limit: '',
    min_confidence_threshold: '',
    execution_cutoff: '',
    max_execute_per_account_per_cycle: '',
  });

  const [lanesForm, setLanesForm] = useState({}); // lane -> {enabled, max_daily_trades}
  ...
  const fetchControls = useCallback(async () => {
    try {
      const [riskRes, tsRes, lanesRes] = await Promise.all([
        fetch(`${API_BASE}/config/risk`),
        fetch(`${API_BASE}/config/trade-selection`),
        fetch(`${API_BASE}/config/accounts`),
      ]);

      if (riskRes.ok) setRiskCaps(await riskRes.json());
      if (tsRes.ok) setTradeSelection(await tsRes.json());
      if (lanesRes.ok) setAccountLanes(await lanesRes.json());
      setControlsError(null);
    } catch (e) {
      setControlsError('Failed to load runtime controls');
    }
  }, []);

  const fetchCommandPack = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/help/command-pack`);
      if (!res.ok) {
        setCommandPackText(null);
        return;
      }
      setCommandPackText(await res.text());
    } catch (e) {
      setCommandPackText(null);
    }
  }, []);
  ...
  useEffect(() => {
    const d = riskCaps?.data;
    if (!d) return;
    setRiskForm({
      max_positions: String(d.max_positions ?? ''),
      max_daily_trades_per_account: String(d.max_daily_trades_per_account ?? ''),
    });
  }, [riskCaps]);

  useEffect(() => {
    const d = tradeSelection?.data?.trade_selection;
    if (!d) return;
    setTradeSelForm({
      mode: String(d.mode ?? ''),
      daily_trade_limit: String(d.daily_trade_limit ?? ''),
      min_confidence_threshold: String(d.min_confidence_threshold ?? ''),
      execution_cutoff: String(d.execution_cutoff ?? ''),
      max_execute_per_account_per_cycle: String(d.max_execute_per_account_per_cycle ?? ''),
    });
  }, [tradeSelection]);

  useEffect(() => {
    const lanes = accountLanes?.data?.lanes;
    if (!Array.isArray(lanes)) return;
    const next = {};
    for (const lane of lanes) {
      next[lane.lane] = {
        enabled: Boolean(lane.enabled),
        max_daily_trades: lane.max_daily_trades === null || lane.max_daily_trades === undefined ? '' : String(lane.max_daily_trades),
      };
    }
    setLanesForm(next);
  }, [accountLanes]);
  ...
  const saveRiskCaps = async () => {
    try {
      setControlsError(null);
      const body = {
        max_positions: Number(riskForm.max_positions),
        max_daily_trades_per_account: Number(riskForm.max_daily_trades_per_account),
      };
      const res = await apiPatchJson('/config/risk', body);
      if (!res.ok) {
        const msg = (await res.json().catch(() => null))?.detail || 'Risk caps update failed';
        setControlsError(msg);
        showToast('Failed to save risk caps', 'danger');
        return;
      }
      setRiskCaps(await res.json());
      showToast('Risk caps saved', 'success');
    } catch (e) {
      setControlsError('Risk caps update failed');
      showToast('Failed to save risk caps', 'danger');
    }
  };

  const saveTradeSelection = async () => {
    try {
      setControlsError(null);
      const body = {
        mode: tradeSelForm.mode || undefined,
        daily_trade_limit: tradeSelForm.daily_trade_limit === '' ? undefined : Number(tradeSelForm.daily_trade_limit),
        min_confidence_threshold: tradeSelForm.min_confidence_threshold === '' ? undefined : Number(tradeSelForm.min_confidence_threshold),
        execution_cutoff: tradeSelForm.execution_cutoff || undefined,
        max_execute_per_account_per_cycle:
          tradeSelForm.max_execute_per_account_per_cycle === '' ? undefined : Number(tradeSelForm.max_execute_per_account_per_cycle),
      };
      const res = await apiPatchJson('/config/trade-selection', body);
      if (!res.ok) {
        const msg = (await res.json().catch(() => null))?.detail || 'Trade selection update failed';
        setControlsError(msg);
        showToast('Failed to save trade selection', 'danger');
        return;
      }
      setTradeSelection(await res.json());
      showToast('Trade selection saved', 'success');
    } catch (e) {
      setControlsError('Trade selection update failed');
      showToast('Failed to save trade selection', 'danger');
    }
  };

  const saveAccountLanes = async () => {
    try {
      setControlsError(null);
      const lanesPayload = {};
      for (const [lane, v] of Object.entries(lanesForm || {})) {
        lanesPayload[lane] = {
          enabled: Boolean(v.enabled),
          max_daily_trades: v.max_daily_trades === '' ? null : Number(v.max_daily_trades),
        };
      }
      const res = await apiPatchJson('/config/accounts', { lanes: lanesPayload });
      if (!res.ok) {
        const msg = (await res.json().catch(() => null))?.detail || 'Accounts update failed';
        setControlsError(msg);
        showToast('Failed to save account lanes', 'danger');
        return;
      }
      setAccountLanes(await res.json());
      showToast('Account lanes saved', 'success');
    } catch (e) {
      setControlsError('Accounts update failed');
      showToast('Failed to save account lanes', 'danger');
    }
  };
  ...
  return (
    <div className="cp-dashboard min-h-screen bg-slate-50 text-slate-800 font-sans pb-12">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        {/* Runner Runtime Controls + Command Pack */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6" aria-label="Runner Runtime Controls">
          <Card title="Runner Controls (Runtime Config)" icon={ShieldAlert} className={isOffline ? 'opacity-60 grayscale pointer-events-none select-none' : ''}>
            <div className="space-y-5">
              {controlsError && (
                <div className="bg-rose-50 border border-rose-200 rounded-lg p-3 text-rose-700 text-sm">
                  <span className="font-bold">Config update failed:</span> {controlsError}
                </div>
              )}

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase">Admin Token (not stored)</label>
                <input
                  type="password"
                  className="w-full mt-1 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                  placeholder="Bearer token for PATCH endpoints"
                  value={adminToken}
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="text-xs font-bold text-slate-800">Risk Caps</div>
                    <div className="text-[10px] text-slate-500 font-mono">
                      Updated: {riskCaps?.data?.updated_at || 'unknown'}
                    </div>
                  </div>
                  <button
                    onClick={saveRiskCaps}
                    disabled={isOffline}
                    className="px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase">max_positions</label>
                    <input
                      type="number"
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      value={riskForm.max_positions}
                      onChange={(e) => setRiskForm({ ...riskForm, max_positions: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase">max_daily_trades_per_account</label>
                    <input
                      type="number"
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      value={riskForm.max_daily_trades_per_account}
                      onChange={(e) => setRiskForm({ ...riskForm, max_daily_trades_per_account: e.target.value })}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="text-xs font-bold text-slate-800">Trade Selection</div>
                    <div className="text-[10px] text-slate-500 font-mono">
                      Updated: {tradeSelection?.data?.updated_at || 'unknown'}
                    </div>
                  </div>
                  <button
                    onClick={saveTradeSelection}
                    disabled={isOffline}
                    className="px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Save className="w-3 h-3" /> Save
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white"
                      value={tradeSelForm.mode}
                      onChange={(e) => setTradeSelForm({ ...tradeSelForm, mode: e.target.value })}
                    >
                      <option value="">(unchanged)</option>
                      <option value="SPEED">SPEED</option>
                      <option value="QUALITY_OVER_SPEED">QUALITY_OVER_SPEED</option>
                      <option value="TOP_N_DAILY">TOP_N_DAILY</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase">execution_cutoff</label>
                    <select
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white"
                      value={tradeSelForm.execution_cutoff}
                      onChange={(e) => setTradeSelForm({ ...tradeSelForm, execution_cutoff: e.target.value })}
                    >
                      <option value="">(unchanged)</option>
                      <option value="NY_CLOSE">NY_CLOSE</option>
                      <option value="IMMEDIATE">IMMEDIATE</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase">daily_trade_limit</label>
                    <input
                      type="number"
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      value={tradeSelForm.daily_trade_limit}
                      onChange={(e) => setTradeSelForm({ ...tradeSelForm, daily_trade_limit: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase">min_confidence_threshold</label>
                    <input
                      type="number"
                      step="0.01"
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      value={tradeSelForm.min_confidence_threshold}
                      onChange={(e) => setTradeSelForm({ ...tradeSelForm, min_confidence_threshold: e.target.value })}
                    />
                  </div>
                  <div className="col-span-2">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">max_execute_per_account_per_cycle</label>
                    <input
                      type="number"
                      className="w-full mt-0.5 text-xs border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      value={tradeSelForm.max_execute_per_account_per_cycle}
                      onChange={(e) => setTradeSelForm({ ...tradeSelForm, max_execute_per_account_per_cycle: e.target.value })}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="text-xs font-bold text-slate-800">Accounts / Lanes</div>
                    <div className="text-[10px] text-slate-500 font-mono">
                      Updated: {accountLanes?.data?.updated_at || 'unknown'}
                    </div>
                  </div>
                  <button
                    onClick={saveAccountLanes}
                    disabled={isOffline}
                    className="px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Save className="w-3 h-3" /> Save
                ...
                {!accountLanes?.data?.lanes || accountLanes.data.lanes.length === 0 ? (
                  <div className="text-xs text-slate-500">No lanes available (backend returned empty).</div>
                ) : (
                  <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                          <th className="px-3 py-2 text-left font-bold text-slate-600">Lane</th>
                          <th className="px-3 py-2 text-left font-bold text-slate-600">Enabled</th>
                          <th className="px-3 py-2 text-left font-bold text-slate-600">Daily Limit Override</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {accountLanes.data.lanes.map((lane) => (
                          <tr key={lane.lane}>
                            <td className="px-3 py-2 font-mono text-slate-700">{lane.lane}</td>
                            <td className="px-3 py-2">
                              <input
                                type="checkbox"
                                checked={Boolean(lanesForm?.[lane.lane]?.enabled)}
                                onChange={(e) =>
                                  setLanesForm({
                                    ...lanesForm,
                                    [lane.lane]: { ...(lanesForm?.[lane.lane] || {}), enabled: e.target.checked },
                                  })
                                }
                              />
                            </td>
                            <td className="px-3 py-2">
                              <input
                                type="number"
                                placeholder="null"
                                className="w-28 text-xs border border-slate-200 p-1.5 rounded focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                                value={lanesForm?.[lane.lane]?.max_daily_trades ?? ''}
                                onChange={(e) =>
                                  setLanesForm({
                                    ...lanesForm,
                                    [lane.lane]: { ...(lanesForm?.[lane.lane] || {}), max_daily_trades: e.target.value },
                                  })
                                }
                              />
                            </td>
          ...
          <Card title="Command Pack" icon={Terminal}>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs text-slate-500">Rendered from `docs/ACCOUNT_CONTROL_COMMAND_PACK.md`</div>
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      try {
                        if (commandPackText) {
                          await navigator.clipboard.writeText(commandPackText);
                          showToast('Command Pack copied', 'success');
                        }
                      } catch (e) {
                        showToast('Copy failed', 'danger');
                      }
                    }}
                    className="text-xs bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-200 font-bold border border-slate-200"
                  >
                    Copy
                  </button>
                  <a
                    href={`${API_BASE}/help/command-pack/download`}
                    className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg hover:bg-indigo-100 font-bold border border-indigo-100"
                  >
                    Download .md
                  </a>
                </div>
              ...
              {!commandPackText ? (
                <div className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded p-3">
                  Command Pack not available (missing file or endpoint returned non-200).
                </div>
              ) : (
                <pre className="text-xs font-mono bg-slate-900 text-slate-100 rounded-lg p-4 overflow-auto max-h-[420px] border border-slate-800">
{commandPackText}
                </pre>
              )}
            </div>
        ...
        {/* Bridge Estate Overview - explicit separation of canonical vs sidecar */}
        <section className="space-y-6" aria-label="Bridge Estate Overview">
          <h2 className="text-lg font-bold text-slate-800">Bridge Estate Overview</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <CriticalCanonicalBridgePanel bridgeStatus={bridgeStatus} />
            <WindowsSidecarPanel />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <BridgeComparisonTable
              canonicalStatus={bridgeStatus}
              sidecarStatus={null}
            />
            <BridgeSwitchStatePanel />
          </div>
        ...
        <section aria-label="Session Gate & Reasoning">
          <Card
            title="Session Gate & Reasoning"
            icon={ShieldAlert}
          >
            {!reasoningSnapshot ? (
              <div className="text-center py-6 text-slate-400 text-sm">
                <ShieldAlert className="w-6 h-6 mx-auto mb-2 opacity-25" />
                <p>No reasoning snapshot available yet. This will populate after the first scan cycle completes.</p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex items-center justify-between text-xs text-slate-500 mb-4">
                  <div>
                    ...
                    <span className="font-mono">{reasoningSnapshot.instrument || 'UNKNOWN'}</span>
                  </div>
                  <div>
                    <span className="font-semibold">Timestamp:</span>{' '}
                    {reasoningSnapshot.timestamp_utc ? new Date(reasoningSnapshot.timestamp_utc).toLocaleTimeString() : 'Unknown'}
                  </div>
                </div>
                
                {/* Primary Blocker & Nearest Unblock Event (Session Gate) */}
                <div className="bg-slate-50 rounded-lg border border-slate-200 p-4">
                  ...
                  <div className="space-y-2">
                    <div>
                      <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Primary Blocker:</span>
                      <div className="mt-1 text-sm font-mono text-slate-800">
                        {reasoningSnapshot.primary_blocker || (
                          <span className="text-emerald-600 font-bold">None - Ready to Trade</span>
                        )}
                      </div>
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Nearest Unblock Event:</span>
                      <div className="mt-1 text-sm font-mono text-slate-800">
                        {reasoningSnapshot.nearest_unblock_event || 'Unknown'}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Readiness Score:</span>
                        <span className="text-lg font-bold font-mono">
                          {typeof reasoningSnapshot.readiness_score === 'number' ? reasoningSnapshot.readiness_score : 0}
                        </span>
                      </div>
                      <div className="h-2 rounded-full bg-slate-200 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${
                            reasoningSnapshot.readiness_score >= 80
                              ? 'bg-emerald-500'
                              : reasoningSnapshot.readiness_score >= 60
                              ? 'bg-amber-500'
                              : 'bg-rose-500'
                          }`}
                          style={{ width: `${Math.max(0, Math.min(100, reasoningSnapshot.readiness_score || 0))}%` }}
                        />
                      </div>
                    </div>
                    
                    {reasoningSnapshot.secondary_blockers && reasoningSnapshot.secondary_blockers.length > 0 && (
                      <div>
                        <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 block">Secondary Blockers:</span>
                        <div className="space-y-1">
                          {reasoningSnapshot.secondary_blockers.map((blocker, idx) => (
                            <div
                              key={idx}
                              className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded px-2 py-1"
                            >
                              {blocker}
                            </div>
                          ))}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        <span className="font-semibold text-slate-600">Regime:</span>{' '}
                        <span className="font-mono">{reasoningSnapshot.regime || 'UNKNOWN'}</span>
                      </div>
                      <div>
                        <span className="font-semibold text-slate-600">Bias State:</span>{' '}
                        <span className="font-mono">{reasoningSnapshot.bias_state || 'UNKNOWN'}</span>
                      </div>
                      <div>
                        <span className="font-semibold text-slate-600">Embargo Active:</span>{' '}
                        <span className="font-mono">{reasoningSnapshot.embargo_active ? 'Yes' : 'No'}</span>
                      </div>
                      {reasoningSnapshot.estimated_time_to_readiness !== null && reasoningSnapshot.estimated_time_to_readiness !== undefined && (
                        <div>
                          <span className="font-semibold text-slate-600">Est. Time to Readiness:</span>{' '}
                          <span className="font-mono">{reasoningSnapshot.estimated_time_to_readiness}m</span>
                        </div>
                      )}
                    </div>
                  </div>

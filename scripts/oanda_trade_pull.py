...
This is the VM version of the local authoritative pull.

Output:
    data/processed/vm_trades_flat.json (Ready for stats_engine)
    data/raw/{account_id}_trades.json (Raw backup)

Usage:
    python scripts/oanda_trade_pull.py
"""
...
def normalize_trade(trade: Dict[str, Any], account_id: str) -> Dict[str, Any]:
    ...
    # current_units should be 0 for closed trades
    ...
    # Strategy extraction (if available in clientExtensions or tags)
    strategy = None
    client_extensions = trade.get('clientExtensions')
    if client_extensions:
        strategy = client_extensions.get('id') or client_extensions.get('tag')
...
def main():
    ...
    print("=" * 70)
    print("📥 VM OANDA Authoritative Trade Pull")
    print("USING AUTHORITATIVE OANDA /trades ENDPOINT")
    print("=" * 70)
    ...
    for account_id in account_ids:
        ...
        # Save Raw
        raw_file = raw_dir / f"{account_id}_trades_vm.json"

...
class WorkingTradingSystem:
    def __init__(self):
        ...
        self._last_trade_selection_config_log_at = 0.0
    ...
    def _load_runtime_config(self) -> None:
        ...
        try:
            ...
            # Keep last-known-good config snapshot (for deterministic behavior)
            if isinstance(config_dict, dict):
                self._last_good_runtime_config_dict = config_dict
            ...
            if isinstance(config_dict, dict):
                self.trade_selector.update_config(config_dict)  # selector unwraps trade_selection
            else:
                # fallback: dataclass-like
                if hasattr(config, 'trade_selection'):
                    self.trade_selector.update_config(config.trade_selection)
            try:
                self._maybe_log_trade_selection_config(force=True)
            except Exception:
                pass
    ...
    def _maybe_log_trade_selection_config(self, force: bool = False) -> None:
        """Log a 1-line TradeSelector config summary at INFO at most once per hour."""
        try:
            now_ts = time.time()
            last_ts = getattr(self, "_last_trade_selection_config_log_at", 0.0) or 0.0
            if not force and (now_ts - last_ts) < 3600:
                return
            cfg = getattr(getattr(self, "trade_selector", None), "config", {}) or {}
            logger.info(
                "Trade Selection Config: "
                f"mode={cfg.get('mode')} "
                f"cutoff={cfg.get('execution_cutoff')} "
                f"min_conf={cfg.get('min_confidence_threshold')} "
                f"max_per_cycle={cfg.get('max_execute_per_account_per_cycle')}"
            )
            self._last_trade_selection_config_log_at = now_ts
        except Exception:
            return
    ...
    def _check_config_reload(self) -> None:
        ...
        try:
            ...
            if current_mtime > self._config_last_mtime:
                ...
                # Update TradeSelector config
                if isinstance(config_dict, dict):
                    self.trade_selector.update_config(config_dict)
                else:
                    if hasattr(config, 'trade_selection'):
                        self.trade_selector.update_config(config.trade_selection)
                try:
    ...
    def scan_and_execute(self):
        ...
        if self._config_load_error:
            logger.error(f"CONFIG_INVALID_BLOCK error={str(self._config_load_error)[:200]}")
            self._write_status_snapshot(0, 0, [], [])
            return 0

        # Optional: 1-line selector config summary (at most once per hour)
        try:
            self._maybe_log_trade_selection_config(force=False)
        except Exception:
            pass

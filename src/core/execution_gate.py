...
class ExecutionGuard:
    ...
    def decision(self, readiness_score: Optional[int] = None) -> ExecutionDecision:
        """
        Make execution decision based on canonical controls.
        
        Args:
            readiness_score: Optional readiness score (0-100).
                           If provided and < 80 in paper mode, blocks execution.
        ...
        6. Readiness check (Paper only) => block if readiness_score < 80 (READINESS_BELOW_THRESHOLD)
        ...
        if trading_mode != "live":
            ...
            if readiness_score is not None and readiness_score < 80:
                return ExecutionDecision(
                    allowed=False,
                    mode="paper",
                    reason_code="READINESS_BELOW_THRESHOLD",
    ...
    def place_market_order(
        ...
        *,
        ...
        """
        # Validate Strategy ID (Phase 1 requirement)
        ...
        try:
            ...
            if not validate_strategy_key(strategy_id):
                logger.error(
                    f"BLOCKED: Invalid strategy_id '{strategy_id}' for {instrument}. "
                    f"Account: {account_id}"
                )
                raise RuntimeError(f"Execution blocked: Invalid strategy_id '{strategy_id}'")
        except ImportError:
            logger.warning("Could not import strategy_registry for validation (allowing for safety)")
            pass

        logger.info(
            f"Execution request: {instrument} {units} units, Strategy: {strategy_id}, "
            f"Account: {account_id}"
        )

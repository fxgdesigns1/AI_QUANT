...
class TradeSelectionSettings:
    ...
    # Selector robustness & spam control
    confidence_missing_policy: str = "reject"  # allow|reject
    max_execute_per_account_per_cycle: int = 1
    executed_ttl_seconds: int = 6 * 60 * 60  # 6 hours
...
class RuntimeConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.trade_selection.execution_cutoff not in ["NY_CLOSE", "IMMEDIATE"]:
            errors.append(f"trade_selection.execution_cutoff must be NY_CLOSE or IMMEDIATE, got {self.trade_selection.execution_cutoff}")

        if self.trade_selection.confidence_missing_policy not in ["allow", "reject"]:
            errors.append(
                f"trade_selection.confidence_missing_policy must be allow or reject, got {self.trade_selection.confidence_missing_policy}"
            )

        if not 1 <= int(self.trade_selection.max_execute_per_account_per_cycle) <= 20:
            errors.append(
                "trade_selection.max_execute_per_account_per_cycle must be 1-20, got "
                f"{self.trade_selection.max_execute_per_account_per_cycle}"
            )

        if int(self.trade_selection.executed_ttl_seconds) < 60:
            errors.append(
                "trade_selection.executed_ttl_seconds must be >= 60, got "
                f"{self.trade_selection.executed_ttl_seconds}"
            )

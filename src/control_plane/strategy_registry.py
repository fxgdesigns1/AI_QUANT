...
# Used to resolve unregistered strategy keys to their canonical equivalents
...
def resolve_strategy_alias(key: str) -> str:
    ...
    Args:
        key: Strategy key (may be alias or canonical)
        
    Returns:
        Canonical strategy key (or original key if no alias found)
    """
...
def get_strategy_info(key: str) -> StrategyInfo | None:
    """Get info for specific strategy (with alias resolution)
    
    Args:
        key: Strategy key (may be alias or canonical)
        
    Returns:
        StrategyInfo if found (after alias resolution), None otherwise
    """
    # Resolve alias first
    resolved_key = resolve_strategy_alias(key)
    return STRATEGIES.get(resolved_key)


def validate_strategy_key(key: str) -> bool:
    """Check if strategy key is valid (with alias resolution)
    
    Args:
        key: Strategy key (may be alias or canonical)
        
    Returns:
        True if key is valid (after alias resolution), False otherwise
    """
    resolved_key = resolve_strategy_alias(key)
    return resolved_key in STRATEGIES
...
def log_strategy_registry() -> None:
    ...
    # Log strategy aliases
    if STRATEGY_ALIASES:
        logger.info("=" * 60)
        logger.info("STRATEGY ALIASES (Legacy/Variant Key Resolution)")
        logger.info("=" * 60)
        for alias, canonical in sorted(STRATEGY_ALIASES.items()):
            logger.info(f"  {alias:30s} -> {canonical}")
        logger.info("=" * 60)
